  ...
  return (
    <View style={{ width: screenWidth - 35}}>
      <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#E4E0E0', borderRadius: 20, paddingVertical: 15, paddingHorizontal: 10 }}>
        <View style={{marginHorizontal: 10}}><Search size={16} color='#333'/></View>
        <TextInput
          placeholder="Search"
          value={searchQuery}
      ...
      {isDropdownVisible && (
        <Text style={{borderBottomWidth: 1, borderColor: '#ccc', marginLeft: 30, marginTop: 40, fontSize: 20}} onPress={() => handleUserSelect(searchResults)}>
          {searchResults}
        </Text>
      )}
    </View>
